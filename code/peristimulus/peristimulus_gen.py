...
def load_bids_events(onset_path, beh_path):
    ...
    onsets['onset'] = onsets['onset'] - 7.92
...
for run in runs:
    onset = f'/srv/lab/fmri/mofomic/uva/{subject}/func/{subject}_task-mfv_run-0{str(run)}_events.tsv'
    beh = f'/srv/lab/fmri/mofomic/uva/{subject}/beh/{subject}_task-mfv_run-0{str(run)}_beh.tsv'
    # Get trial information for run and subject
    dm = load_bids_events(onset, beh)
    # Load brain and smooth
    brain_data = Brain_Data(f'/srv/lab/fmri/mofomic/uva/derivatives/fmriprep/{subject}/{subject}/func/{subject}_task-mfv_run-0{str(run)}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz').smooth(8)[11:-11]
...
df.to_csv(f'/srv/lab/fmri/mft/fhopp_diss/analysis/signature/results/peristimulus/generalization/{subject}.csv')