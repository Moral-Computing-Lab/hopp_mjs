...
xval = pd.read_csv('/srv/lab/fmri/mft/fhopp_diss/analysis/signature/weightmaps/mjs/mjs_xval_split.csv')
xval_ix = int(xval.loc[:, (xval[xval['Subject']==subject]==0).all()].columns[0].split('_')[1])
print("XVAL Index: ", xval_ix)

# Select weight map where subject was *not* part of training 
mjs_xval_map = Brain_Data('/srv/lab/fmri/mft/fhopp_diss/analysis/signature/weightmaps/mjs/mjs_weights_xval.nii.gz')[xval_ix]
...
for run in runs:
    onset = f'/srv/lab/fmri/mofomic/ucsb_1/{subject}/func/{subject}_task-mfv_run-0{str(run)}_events.tsv'
    beh = f'/srv/lab/fmri/mofomic/ucsb_1/{subject}/beh/{subject}_task-mfv_run-0{str(run)}_beh.tsv'
    # Get trial information for run and subject
    dm = load_bids_events(onset, beh)
    # Load brain and smooth
    brain_data = Brain_Data(f'/srv/lab/fmri/mofomic/ucsb_1/derivatives/fmriprep/{subject}/{subject}/func/{subject}_task-mfv_run-0{str(run)}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz').smooth(8)[11:-11]
    ...
    for i,row in dm.iterrows():
        ...
        for tr_ix, tr_brain in enumerate(trial_brain):
            pexp.at[row_count, 'pexp'] = tr_brain.similarity(mjs_xval_map, method='dot_product')
...
df.to_csv(f'/srv/lab/fmri/mft/fhopp_diss/analysis/signature/results/peristimulus/discovery/{subject}.csv')