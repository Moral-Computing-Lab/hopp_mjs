...
def load_bids_events(onset_path, beh_path, n_tr):
    ...
    beh = pd.read_csv(beh_path, sep='\t', usecols=['stim_file', 'moral_decision'])
    onsets = onsets.merge(beh, on='stim_file')
    
    onsets = onsets[onsets['trial_type'] != 'cross'] # cross is modeled as implicit baseline.

    # Identify number where moral_decision is null
    null_decision_na = onsets[onsets['moral_decision'].isnull()]['stim_file'].values
    # Identify number where moral_decision is not null
    onsets = onsets[~onsets['stim_file'].isin(null_decision_na)]

    for i,row in onsets.iterrows():
        if row['trial_type'] == 'stimulus':
            onsets.at[i, 'trial_type'] = str(int(row['moral_decision']))
        if row['trial_type'] == 'decision':
            onsets.at[i, 'trial_type'] = "decision"
...
# Set up different design matrices for different analyses
...
for run in range(first_run, num_runs+1):
    ...
    nifti_path = f'/srv/lab/fmri/mofomic/{study}/derivatives/fmriprep/{subject}/{subject}/func/{subject}_task-smid_run-0{str(run)}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz'
    cov_path = f'/srv/lab/fmri/mofomic/{study}/derivatives/fmriprep/{subject}/{subject}/func/{subject}_task-smid_run-0{str(run)}_desc-confounds_timeseries.tsv'
    onset_path = f'/srv/lab/fmri/mofomic/{study}/{subject}/func/{subject}_task-smid_run-0{str(run)}_events.tsv'
    beh_path = f'/srv/lab/fmri/mofomic/{study}/{subject}/beh/{subject}_task-smid_run-0{str(run)}_beh.tsv'
...
for i, col in enumerate(fmri_data.X.columns):
    for rating in ['1_c0','2_c0','3_c0','4_c0','5_c0']:
        if col == rating:
            print(f'Rating from design matrix column: {i, col}')
            stats['beta'][i].write(output_dir + f"{subject}_smid_rating{col.split('_')[0]}_smooth{fwhm}.nii.gz")