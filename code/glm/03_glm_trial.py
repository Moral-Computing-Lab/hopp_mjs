# General Linear Model (GLM) for Moral Judgment Beta Maps for Each Vignette Stimulus
# This is the Least Squares All (LSA) approach
# Discovery Cohort
...
def load_bids_events(onset_path, beh_path, n_tr):
    ...
    onsets = pd.read_csv(onset_path, sep='\t',usecols=['onset','duration','cond_id'])

    if study == 'duke':
        onsets = pd.read_csv(onset_path, sep='\t',usecols=['trialstart_zeroed','duration','cond_id'])
        onsets = onsets.rename(columns={'trialstart_zeroed':'onset'})
        onsets = onsets[['onset','duration','cond_id']]
    ...
    onsets['trial_type'] = 'vig_'+ onsets['cond_id']
...
if study == 'ucsb_1' and subject == 'sub-35':
    # clean breaks column order for sub-35, so no cleaning applied here. 
    fmri_data.X = all_runs
else:
    fmri_data.X = all_runs.clean()

if study == 'ucsb_2':
    if subject == 'sub-13' or subject == 'sub-21':
        # clean breaks column order for sub-13 & sub-21, so no cleaning applied here. 
        fmri_data.X = all_runs
    else:
        fmri_data.X = all_runs.clean()
...
# Save beta maps for each vignette
for i, col in enumerate(fmri_data.X.columns):
    if col.startswith('vig'):
        print(f'Vignette from design matrix column: {col}')
        stats['beta'][i].write(output_dir + f"{subject}_item_{col}_smooth{fwhm}.nii.gz")