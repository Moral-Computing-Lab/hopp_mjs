# General Linear Model (GLM) for Moral Judgment Beta Maps for Each MFV Condition
# Discovery Cohort
...
def load_bids_events(onset_path, beh_path, n_tr):
    ...
    onsets['trial_type'] = onsets['trial_type']  + '_' + onsets['moral_decision'].astype(int).astype(str)
    onsets['trial_type'] = 'cond_' + onsets['trial_type']
...
for i, col in enumerate(fmri_data.X.columns):
    if col.startswith('cond'):
        print(f'Beta from design matrix column: {col}')
        stats['beta'][i].write(output_dir + f"{subject}_vig_{col}_smooth{fwhm}.nii.gz")